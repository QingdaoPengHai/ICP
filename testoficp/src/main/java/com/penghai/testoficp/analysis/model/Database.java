...
public class Database {
	...
	public String conbineCreateTableSQL(){
		...
		for(Table table : tables){
			//指定执行的数据库
			sql.append(" USE ").append(this.name).append(";").append(table.createTableSQL());
		}
	...
	public String conbineCreateForeignKeySQL(){
		...
		for(Table table : tables){
			if(!"".equals(table.createForeignKeySQL())){
				//指定执行的数据库
				sql.append(" USE ").append(this.name).append(";").append(table.createForeignKeySQL());
			}
		}
		return sql.toString();
	}
